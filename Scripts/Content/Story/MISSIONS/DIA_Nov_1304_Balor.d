...
var int Balor_RipOff;

instance DIA_Balor_RipOffNewCamp(C_Info)
{
	npc = NOV_1304_Balor;
	nr = 1;
	condition = DIA_Balor_RipOffNewCamp_Condition;
	information = DIA_Balor_RipOffNewCamp_Info;
	permanent = 1;
	description = "50 bry³ek to uczciwa cena. Proszê bardzo.";
};

func int DIA_Balor_RipOffNewCamp_Condition()
{
	if(Npc_KnowsInfo(hero,DIA_Balor_RipOff) && Balor_PlayerCheating == TRUE && Balor_RipOff == FALSE)
	{
		return 1;
	};
};

func void DIA_Balor_RipOffNewCamp_Info()
{
	AI_Output(other,self,"DIA_Balor_TellDealer_Pay_15_00");	//50 bry³ek to uczciwa cena. Proszê bardzo.
	if(Npc_HasItems(other,ItMiNugget) >= 50)
	{
		Info_ClearChoices(DIA_Balor_RipOffNewCamp);
		B_GiveInvItems(other,self,ItMiNugget,50);
		CreateInvItems(self,ItMi_Plants_Swampherb_01,50);
		B_GiveInvItems(self,hero,ItMi_Plants_Swampherb_01,50);
		Balor_RipOff = TRUE;
