...
FUNC VOID DIA_Wolf_SellArmor_Info()
{
	...
	if ( (Npc_GetTrueGuild(other)==GIL_ORG) || (Npc_GetTrueGuild(other)==GIL_SLD) )
	{
		...
		Info_Addchoice 		(DIA_Wolf_SellArmor,B_BuildBuyArmorString(NAME_WolfHeavyBandits,VALUE_ORG_ARMOR_M),DIA_Wolf_SellArmor_M);
	}
	else
	{
...
instance  ORG_855_Wolf_TRAIN (C_INFO)
{
	...
	permanent		= 1;
	description		= "£uk +3% (50 bry³ek rudy, 10pkt. umiejêtnoœci)";
};

FUNC int  ORG_855_Wolf_TRAIN_Condition()
{	
	if (Npc_KnowsInfo (hero,ORG_855_Wolf_TRAINOFFER))
	&& (Npc_GetTalentValue(hero, NPC_TALENT_BOW) < 30 )
	{
		return TRUE;
	};
